...
GID_DESC_LINEAL   = 2064367150
GID_DESC_PRODUCTO = 1239169673
...
TIPO_LABELS = {
    'sellout':      'Descuento SELL-OUT',
    'ct_lineal':    'Venta Clientes Descuento Cabecera',
    'farmago':      'Venta Farmago Descuento Cabecera',
    'farmatencion': 'Venta Farmatención Descuento Cabecera',
    'dia_laboratorio': 'Descuento Día Laboratorio',
}
...
PRESET_CONFIG_LAB = {
    ...
    'farma, s.a': {'excluir': True,  'costo': False},
...
# ─────────────────────────────────────────────
# Permite que distintos nombres de laboratorio en Odoo (variantes de razón
# social, abreviaciones, etc.) se traten como el MISMO laboratorio para
# efectos de matcheo contra el Sheets CT/Día Laboratorio y de configuración
# (Excluir/A Costo), sin tener que duplicar la entrada en el Sheets.
#
# Clave: nombre CANÓNICO (el que se usa para mostrar en reportes/Sheets/correos).
# Valor: lista de variantes adicionales que existen en Odoo y deben resolver
# al mismo laboratorio.
ALIAS_LABORATORIOS = {
    'laboratorios farma, s.a.': ['farma, s.a'],
    # agrega más líneas según se detecten otras variantes, ej.:
    # 'laboratorios x, c.a.': ['x, c.a.', 'x c.a.'],
}

def _construir_mapa_alias(alias_dict):
    """
    Invierte ALIAS_LABORATORIOS a un mapa {variante_normalizada: clave_canonica},
    incluyendo la propia clave canónica (para que se resuelva a sí misma).
    """
    mapa = {}
    for canonico, variantes in alias_dict.items():
        canonico_norm = _normalizar_nombre_lab(canonico)
        mapa[canonico_norm] = canonico_norm
        for v in variantes:
            mapa[_normalizar_nombre_lab(v)] = canonico_norm
    return mapa

MAPA_ALIAS_LAB = _construir_mapa_alias(ALIAS_LABORATORIOS)

# Nombre "bonito" a mostrar para cada clave canónica (reportes/Sheets/correos)
NOMBRES_CANONICOS = {
    'laboratorios farma, s.a.': 'LABORATORIOS FARMA, S.A.'
}

def _lab_key_con_alias(nombre_lab):
    """
    Normaliza un nombre de laboratorio (tal como viene de Odoo o del Sheets)
    y lo resuelve a su clave canónica si existe un alias definido. Si no hay
    alias, devuelve la normalización estándar (sin cambios de comportamiento
    para laboratorios sin variantes conocidas).
    """
    norm = _normalizar_nombre_lab(nombre_lab)
    return MAPA_ALIAS_LAB.get(norm, norm)
...
def _mask_aplica_cliente(aplica_norm, cadena_norm):
    """
    aplica_norm: serie normalizada (sin tildes, lower) de la columna aplica_cliente
                 de la regla (Todos/Farmago/Independientes).
    cadena_norm: serie normalizada de la cadena del cliente de cada línea.
    """
...
def obtener_ct_hoja1(url_base):
    ...
    try:
        ...
        df['laboratorio_key']  = df['laboratorio'].astype(str).apply(_lab_key_con_alias)
...
def obtener_ct_detalle(url_base):
    ...
    try:
        ...
        df['laboratorio_det_key']    = df['laboratorio_det'].apply(_lab_key_con_alias)
...
def obtener_ct_descuento_lineal(url_base):
    """
    Pestaña Descuento Lineal (gid=2064367150):
    A=aplica_cliente (Todos/Farmago/Independientes), B=laboratorio,
    C=descuento(%), D=fecha_inicio, E=fecha_fin.
    Aplica a TODOS los productos del laboratorio indicado, filtrado por la
    categoría de cliente (aplica_cliente) y vigente en el rango de fechas
    de esa fila (no un rango fijo por partner, como en Hoja1).
    """
    try:
        df = pd.read_csv(url_con_gid(url_base, GID_DESC_LINEAL))
        df = df.rename(columns={
            df.columns[0]: 'aplica_cliente_raw',
            df.columns[1]: 'laboratorio',
            df.columns[2]: 'descuento_raw',
            df.columns[3]: 'fecha_inicio',
            df.columns[4]: 'fecha_fin',
        })

        desc_limpio   = df['descuento_raw'].astype(str).str.replace('%', '', regex=False).str.strip()
        filas_validas = pd.to_numeric(desc_limpio, errors='coerce').notna()
        descartadas   = (~filas_validas).sum()
        if descartadas > 0:
            st.caption(f"⚠️ Descuento Lineal CT: {descartadas} fila(s) con descuento no numérico ignoradas.")
        df = df[filas_validas].copy()

        df['descuento_dl']    = desc_limpio[filas_validas].astype(float) / 100
        df['laboratorio_key'] = df['laboratorio'].astype(str).apply(_lab_key_con_alias)
        df['aplica_cliente']  = (
            df['aplica_cliente_raw'].astype(str).str.strip().str.lower().apply(quitar_tildes)
        )
        df['fecha_inicio'] = pd.to_datetime(df['fecha_inicio'], errors='coerce').dt.date
        df['fecha_fin']    = pd.to_datetime(df['fecha_fin'],    errors='coerce').dt.date

        return df[['aplica_cliente', 'laboratorio', 'laboratorio_key',
                    'descuento_dl', 'fecha_inicio', 'fecha_fin']]
    except Exception as e:
        st.warning(f"⚠️ No se pudo leer pestaña Descuento Lineal CT: {e}")
        return pd.DataFrame()


def obtener_ct_descuento_producto(url_base):
    """
    Pestaña Descuento por Producto (gid=1239169673):
    A=aplica_cliente, B=laboratorio (solo referencia visual, no se usa para
    matchear), C=barcode, D=(sin uso), E=descuento(%), F=fecha_inicio,
    G=fecha_fin.
    """
    try:
        df = pd.read_csv(url_con_gid(url_base, GID_DESC_PRODUCTO))
        df = df.rename(columns={
            df.columns[0]: 'aplica_cliente_raw',
            df.columns[1]: 'laboratorio_ref',
            df.columns[2]: 'barcode_dp',
            df.columns[4]: 'descuento_raw',
            df.columns[5]: 'fecha_inicio',
        ...
        descartadas   = (~filas_validas).sum()
        if descartadas > 0:
            st.caption(f"⚠️ Descuento por Producto CT: {descartadas} fila(s) con descuento no numérico ignoradas.")
        df = df[filas_validas].copy()

        df['descuento_dp']    = desc_limpio[filas_validas].astype(float) / 100
        df['barcode_dp']      = estandarizar_barcodes(df['barcode_dp'])
        df['laboratorio_ref'] = df['laboratorio_ref'].astype(str).str.strip()
        df['aplica_cliente']  = (
            df['aplica_cliente_raw'].astype(str).str.strip().str.lower().apply(quitar_tildes)
        )
        df['fecha_inicio'] = pd.to_datetime(df['fecha_inicio'], errors='coerce').dt.date
        ...
        return df[['aplica_cliente', 'laboratorio_ref', 'barcode_dp',
                    'descuento_dp', 'fecha_inicio', 'fecha_fin']]
    except Exception as e:
        st.warning(f"⚠️ No se pudo leer pestaña Descuento por Producto CT: {e}")
        return pd.DataFrame()
    ...
    if url_ct and url_ct.startswith("https://"):
        ...
        if not df_h1.empty:
            ...
            for _, row in df_h1_vig[['laboratorio', 'laboratorio_key']].drop_duplicates().iterrows():
                labs.setdefault(_lab_key_con_alias(row['laboratorio']), row['laboratorio'].strip())
        ...
        if not df_det.empty:
            ...
            for _, row in df_det_vig[['laboratorio_det', 'laboratorio_det_key']].drop_duplicates().iterrows():
                labs.setdefault(_lab_key_con_alias(row['laboratorio_det']), row['laboratorio_det'].strip())
        ...
        df_dl = obtener_ct_descuento_lineal(url_ct)
        if not df_dl.empty:
            df_dl_vig = df_dl[_vigente_en_rango(df_dl['fecha_inicio'], df_dl['fecha_fin'], fecha_inicio, fecha_fin)]
            for _, row in df_dl_vig[['laboratorio', 'laboratorio_key']].drop_duplicates().iterrows():
                labs.setdefault(_lab_key_con_alias(row['laboratorio']), row['laboratorio'].strip())
            n_desc = len(df_dl) - len(df_dl_vig)
            if n_desc > 0:
                st.caption(f"ℹ️ Descuento Lineal CT: {n_desc} regla(s) fuera del rango de fechas, ignoradas.")

        df_dp = obtener_ct_descuento_producto(url_ct)
        if not df_dp.empty:
            df_dp_vig = df_dp[_vigente_en_rango(df_dp['fecha_inicio'], df_dp['fecha_fin'], fecha_inicio, fecha_fin)]
            for _, row in df_dp_vig[['laboratorio_ref']].drop_duplicates().iterrows():
                lab_nombre = row['laboratorio_ref'].strip()
                if lab_nombre:
                    labs.setdefault(_lab_key_con_alias(lab_nombre), lab_nombre)
            n_desc = len(df_dp) - len(df_dp_vig)
            if n_desc > 0:
                st.caption(f"ℹ️ Descuento por Producto CT: {n_desc} regla(s) fuera del rango de fechas, ignoradas.")
                ...
                try:
                    ...
                    for p in data:
                        ...
                        if isinstance(lab, (list, tuple)):
                            ...
                            labs.setdefault(_lab_key_con_alias(nombre), nombre)
                except Exception as e:
                    st.warning(
...
def matchear_ct(df_final, url_ct):
    """
    Devuelve DataFrame con columnas ['id', 'descuento_ct', 'fuente_ct', 'tipo_ct'].
    Una misma línea de factura ('id') puede aparecer HASTA 2 VECES: una vez
    representando el esquema "CT Fijo" (Hoja1/Detalle, tipo_ct en
    {'ct_lineal','farmago','farmatencion'}) y una vez representando el esquema
    "Día Laboratorio" (Descuento Lineal/Producto, tipo_ct='dia_laboratorio').
    Dentro de cada esquema SÍ se deduplica (para no reportar dos veces la
    misma regla del mismo esquema), pero NUNCA se deduplica entre esquemas
    distintos — eso lo decide calcular_descuentos_finales según Libre/Excluir.
    """
    ...
    if df_hoja1.empty and df_detalle.empty and df_dl.empty and df_dp.empty:
    ...
    df['lab_key'] = df['laboratory_name'].apply(
        lambda x: _lab_key_con_alias(x[1] if isinstance(x, (list, tuple)) else str(x))
    )
    ...
    if not df_hoja1.empty:
        ...
        if not h1_cad.empty:
            ...
            if 'id' in m.columns and m.duplicated(subset=['id']).any():
                n_antes_cad = len(m)
                m = m.sort_values('descuento_ct', ascending=False).drop_duplicates(subset=['id'], keep='first')
                n_dedup_cad = n_antes_cad - len(m)
                if n_dedup_cad > 0:
                    st.warning(
                        f"⚠️ Hoja1 CT (Farmago/Farmatención): {n_dedup_cad} línea(s) con rangos "
                        f"de vigencia solapados — se tomó el mayor descuento."
                    )
            if not m.empty:
    ...
    if hoja1_matches:
        ...
        if 'id' in df_hoja1_result.columns:
            df_hoja1_result = df_hoja1_result.drop_duplicates(subset=['id'], keep='first')  # cadena > general, DENTRO de Hoja1
    ...
    if not df_detalle.empty:
        ...
        if partes:
            ...
            if 'id' in df_detalle_result.columns:
                df_detalle_result = df_detalle_result.drop_duplicates(subset=['id'], keep='first')  # cadena > cliente, DENTRO de Detalle
            df_detalle_result['_fuente'] = 'detalle'

    # ── Unión INTERNA del esquema CT Fijo: Detalle gana sobre Hoja1 ──
    ct_fijo_partes = [p for p in [df_hoja1_result, df_detalle_result] if not p.empty]
    ct_fijo_result = pd.DataFrame()
    if ct_fijo_partes:
        ct_fijo_result = pd.concat(ct_fijo_partes, ignore_index=True)
        if 'id' in ct_fijo_result.columns:
            ct_fijo_result = ct_fijo_result.sort_values('_fuente', ascending=True).drop_duplicates(subset=['id'], keep='last')

    # ── Aplicar lista Excluir DENTRO del esquema CT Fijo (Hoja1, Detalle mantiene override) ──
    if not df_excluir.empty and not ct_fijo_result.empty and 'barcode_norm' in ct_fijo_result.columns:
        mask_h1 = ct_fijo_result['_fuente'] == 'hoja1'
        df_h1r  = ct_fijo_result[mask_h1].copy()
        df_det  = ct_fijo_result[~mask_h1].copy()
        if not df_h1r.empty:
        ...
        ct_fijo_result = pd.concat([df_h1r, df_det], ignore_index=True)

    if not ct_fijo_result.empty:
        ct_fijo_result = ct_fijo_result.rename(columns={'_fuente': 'fuente_ct'})

    # ══════════════════════════════════════════════════════════════
    # ESQUEMA "DÍA LABORATORIO": Descuento Lineal + Descuento Producto
    # ══════════════════════════════════════════════════════════════
    dl_result = pd.DataFrame()
    if not df_dl.empty:
        m = df.merge(
            df_dl[['aplica_cliente', 'laboratorio_key', 'descuento_dl', 'fecha_inicio', 'fecha_fin']],
            left_on='lab_key', right_on='laboratorio_key', how='inner'
        )
        if not m.empty:
            mask_vig = (
                m['fecha_inicio'].notna() & m['fecha_fin'].notna() &
                (m['invoice_date_obj'] >= m['fecha_inicio']) &
                (m['invoice_date_obj'] <= m['fecha_fin'])
            )
            mask_cli = _mask_aplica_cliente(m['aplica_cliente'], m['cadena_key_f'])
            m = m[mask_vig & mask_cli].copy()
        if not m.empty:
            if 'id' in m.columns and m.duplicated(subset=['id']).any():
                m['_prioridad_dl'] = np.where(
                    m['aplica_cliente'].str.contains('farmago|independiente', na=False, regex=True), 2, 1
                )
                m = (
                    m.sort_values(['_prioridad_dl', 'descuento_dl'], ascending=[False, False])
                    .drop_duplicates(subset=['id'], keep='first')
                    .drop(columns=['_prioridad_dl'])
                )
            m['tipo_ct'] = 'dia_laboratorio'
            m = m.rename(columns={'descuento_dl': 'descuento_ct'})
            m['_fuente'] = 'desc_lineal'
            dl_result = m

    dp_result = pd.DataFrame()
    if not df_dp.empty:
        m = df.merge(
            df_dp[['aplica_cliente', 'barcode_dp', 'descuento_dp', 'fecha_inicio', 'fecha_fin']],
            left_on='barcode_norm', right_on='barcode_dp', how='inner'
        )
        if not m.empty:
            mask_vig = (
                m['fecha_inicio'].notna() & m['fecha_fin'].notna() &
        ...
        if not m.empty:
            if 'id' in m.columns and m.duplicated(subset=['id']).any():
                m['_prioridad_dp'] = np.where(
                    m['aplica_cliente'].str.contains('farmago|independiente', na=False, regex=True), 2, 1
                )
                m = (
                    m.sort_values(['_prioridad_dp', 'descuento_dp'], ascending=[False, False])
                    .drop_duplicates(subset=['id'], keep='first')
                    .drop(columns=['_prioridad_dp'])
                )
            m['tipo_ct'] = 'dia_laboratorio'
            m = m.rename(columns={'descuento_dp': 'descuento_ct'})
            m['_fuente'] = 'desc_producto'
            dp_result = m

    # ── Unión INTERNA del esquema Día Laboratorio: Producto gana sobre Lineal ──
    dia_lab_partes = [p for p in [dl_result, dp_result] if not p.empty]
    dia_lab_result = pd.DataFrame()
    if dia_lab_partes:
        dia_lab_result = pd.concat(dia_lab_partes, ignore_index=True)
        if 'id' in dia_lab_result.columns:
            dia_lab_result = dia_lab_result.sort_values('_fuente', ascending=True).drop_duplicates(subset=['id'], keep='last')

    # ── Aplicar lista Excluir DENTRO del esquema Día Laboratorio (Lineal, Producto mantiene override) ──
    if not df_excluir.empty and not dia_lab_result.empty and 'barcode_norm' in dia_lab_result.columns:
        mask_dl = dia_lab_result['_fuente'] == 'desc_lineal'
        df_dlr  = dia_lab_result[mask_dl].copy()
        df_dpr  = dia_lab_result[~mask_dl].copy()
        if not df_dlr.empty:
            chk = df_dlr[['barcode_norm', 'lab_key']].merge(
                df_excluir, left_on=['barcode_norm', 'lab_key'],
                right_on=['barcode_excluido', 'laboratorio_excluir_key'], how='left', indicator=True
            )
            mask_excl = (chk['_merge'] == 'both').values
            n_excl = mask_excl.sum()
            if n_excl > 0:
                st.caption(f"ℹ️ {n_excl} línea(s) excluidas de Descuento Lineal por lista CT (Desc.Producto mantiene su override).")
            df_dlr = df_dlr[~mask_excl]
        dia_lab_result = pd.concat([df_dlr, df_dpr], ignore_index=True)

    if not dia_lab_result.empty:
        dia_lab_result = dia_lab_result.rename(columns={'_fuente': 'fuente_ct'})

    # ══════════════════════════════════════════════════════════════
    # UNIÓN FINAL: los dos esquemas COEXISTEN, sin dedupe cruzado
    # ══════════════════════════════════════════════════════════════
    if ct_fijo_result.empty and dia_lab_result.empty:
        return pd.DataFrame(columns=['id', 'descuento_ct', 'fuente_ct', 'tipo_ct'])

    n_hoja1   = (ct_fijo_result['fuente_ct'] == 'hoja1').sum() if not ct_fijo_result.empty else 0
    n_detalle = (ct_fijo_result['fuente_ct'] == 'detalle').sum() if not ct_fijo_result.empty else 0
    n_dl      = (dia_lab_result['fuente_ct'] == 'desc_lineal').sum() if not dia_lab_result.empty else 0
    n_dp      = (dia_lab_result['fuente_ct'] == 'desc_producto').sum() if not dia_lab_result.empty else 0
    st.toast(f"CT: {n_hoja1} Hoja1 · {n_detalle} Detalle · {n_dl} Desc.Lineal · {n_dp} Desc.Producto")

    partes_finales = [p[['id', 'descuento_ct', 'fuente_ct', 'tipo_ct']] for p in [ct_fijo_result, dia_lab_result] if not p.empty]
    return pd.concat(partes_finales, ignore_index=True)
...
def calcular_descuentos_finales(df_bruto, config_lab):
    """
    df_bruto viene en formato LARGO: una fila por cada esquema que matcheó
    esa línea de factura ('id'). Un mismo 'id' puede tener hasta 3 filas:
    una SellOut, una CT Fijo (Hoja1/Detalle) y una Día Laboratorio
    (Desc.Lineal/Producto) — cada una ya trae su propio descuento_valor,
    fuente_final ('so'/'ct') y tipo_reporte.

    Reglas por 'id' (según config_lab[lab]['excluir']):
      - Modo LIBRE (default): se conservan TODAS las filas de ese id — cada
        esquema que matcheó se reporta por separado.
      - Modo EXCLUIR: de todas las filas de ese id (sin importar el esquema),
        se conserva SOLO la de mayor descuento_valor.

    En ambos modos, las filas CT (fuente_final == 'ct') recalculan su precio
    base invertido desde SellOut si esa misma factura-línea también tuvo
    match SellOut (columna 'descuento_so_ref'), independientemente de si esa
    fila sobrevive por Libre o gana por Excluir.
    """
    df = df_bruto.copy()
    df['lab_key'] = df['laboratory_name'].apply(
        lambda x: (x[1] if isinstance(x, (list, tuple)) else str(x)).strip().lower())

    if 'descuento_so_ref' not in df.columns:
        df['descuento_so_ref'] = np.nan

    mask_recalc = (df['fuente_final'] == 'ct') & df['descuento_so_ref'].notna()

    df['price_unit_base'] = df['price_unit']
    df.loc[mask_recalc, 'price_unit_base'] = (
        df.loc[mask_recalc, 'price_unit'] /
        (1 - df.loc[mask_recalc, 'descuento_so_ref'].clip(upper=0.9999))
    )

    df['gano_sellout'] = False
    df.loc[mask_recalc, 'gano_sellout'] = (
        df.loc[mask_recalc, 'descuento_so_ref'] > df.loc[mask_recalc, 'descuento_valor']
    )
    # Filas SellOut puras: informativo, True si esa misma factura-línea también tuvo match CT
    ids_con_ct = set(df.loc[df['fuente_final'] == 'ct', 'id'])
    df.loc[df['fuente_final'] == 'so', 'gano_sellout'] = df.loc[df['fuente_final'] == 'so', 'id'].isin(ids_con_ct)

    df['excluir_lab'] = df['lab_key'].apply(
        lambda k: config_lab.get(k, {'excluir': False, 'costo': False}).get('excluir', False)
    )

    partes = []

    # ── Modo Excluir: UNA sola línea por id, la de mayor descuento entre TODOS los esquemas ──
    df_excl = df[df['excluir_lab']].copy()
    if not df_excl.empty:
        df_excl = (
            df_excl.sort_values('descuento_valor', ascending=False)
            .drop_duplicates(subset=['id'], keep='first')
        )
        partes.append(df_excl)

    # ── Modo Libre: TODAS las filas de ese id se reportan, una por esquema ──
    df_libre = df[~df['excluir_lab']].copy()
    if not df_libre.empty:
        partes.append(df_libre)
    ...
    resultado = resultado.drop(
        columns=['descuento_ct', 'descuento_so', 'tipo_ct', 'excluir_lab', 'descuento_so_ref'],
        errors='ignore'
    )
    return resultado
...
def _escribir_hoja_reporte_con_tipo(workbook, sheet_name, reporte, header_format, percent_format,
                                     num_format_moneda, bold_format):
    """
    Igual que _escribir_hoja_reporte, pero agrega una columna adicional
    'Tipo de Descuento' al final del bloque de datos (antes de Total/Monto NC),
    para la vista combinada por laboratorio.
    """
    worksheet = workbook.add_worksheet(sheet_name)
    monto_format      = workbook.add_format({'num_format': num_format_moneda})
    bold_monto_format = workbook.add_format({'num_format': num_format_moneda, 'bold': True})
    ...
    encabezados = [
        'Fecha Factura', 'ID Cliente', 'Cliente', 'Nro. Factura',
        'Código de Barras', 'Descripción', 'Laboratorio', 'Código Laboratorio',
        'Cantidad', 'Precio', 'Descuento %', 'Tipo de Descuento', 'Total', 'Monto NC'
    ]
    for col_num, value in enumerate(encabezados):
        worksheet.write(0, col_num, value, header_format)

    # Columnas escritas "en bloque" (sin formato especial por celda)
    columnas_datos_simples = [
        'invoice_date', 'partner_id_num', 'partner_id', 'invoice_number_next',
        'barcode', 'name', 'laboratory_name', 'supplier_code',
        'quantity',
    ]
    for row_num, (_, fila) in enumerate(reporte.iterrows()):
        for col_num, campo in enumerate(columnas_datos_simples):
            worksheet.write(row_num + 1, col_num, fila[campo])
        # Precio (J) con formato de moneda
        worksheet.write(row_num + 1, 9, fila['valor_unitario'], monto_format)
        # Descuento % (K) con formato de porcentaje explícito
        worksheet.write(row_num + 1, 10, fila['descuento'], percent_format)
        # Tipo de Descuento (L), texto plano
        worksheet.write(row_num + 1, 11, fila['tipo_descuento'])
        # Total (M) = Precio × Cantidad
        worksheet.write_formula(row_num + 1, 12, f'=J{row_num + 2}*I{row_num + 2}', monto_format)
        # Monto NC (N) = Descuento% × Total
        worksheet.write_formula(row_num + 1, 13, f'=K{row_num + 2}*M{row_num + 2}', monto_format)

    last_row = len(reporte) + 1
    worksheet.write(last_row, 12, "Total NC", bold_format)
    worksheet.write_formula(last_row, 13, f"=SUM(N2:N{last_row})", bold_monto_format)

    columnas_ancho = [
        'invoice_date', 'partner_id_num', 'partner_id', 'invoice_number_next',
        'barcode', 'name', 'laboratory_name', 'supplier_code',
        'quantity', 'valor_unitario', 'descuento', 'tipo_descuento',
    ]
    for i, col in enumerate(columnas_ancho):
        col_data = reporte[col].astype(str).fillna("")
        worksheet.set_column(i, i, max(col_data.map(len).max(), len(encabezados[i])) + 2)
    subtotal   = reporte['quantity'] * reporte['valor_unitario']
    total_desc = subtotal * reporte['descuento']
    worksheet.set_column(12, 12, max(subtotal.astype(str).map(len).max(), 12) + 6)
    worksheet.set_column(13, 13, max(total_desc.astype(str).map(len).max(), 12) + 6)
...
# ─────────────────────────────────────────────
# EXCEL COMBINADO POR LABORATORIO (todos los tipos juntos)
# ─────────────────────────────────────────────
def generar_excel_lab_combinado(df_lab_todo, config_lab):
    """
    Genera un Excel con TODAS las líneas de un solo laboratorio (los 5 tipos
    de reporte juntos: SellOut, CT Lineal, Farmago, Farmatención, Día
    Laboratorio), separado en pestañas "Ventas USD" / "Ventas Bs" igual que
    el consolidado general, pero acotado a este laboratorio. Se agrega una
    columna 'Tipo de Descuento' (texto legible vía TIPO_LABELS) para poder
    identificar el origen de cada línea sin perder esa trazabilidad.
    """
    if df_lab_todo.empty:
        return None
    ...
    df['valor_calculado']  = df.apply(lambda r: calcular_valor_unitario(r, config_lab), axis=1)
    df['es_usd']           = df['currency_id'].apply(_es_moneda_usd)
    df['tipo_descuento']   = df['tipo_reporte'].apply(lambda t: TIPO_LABELS.get(t, t))
    ...
    reporte = pd.DataFrame({
        ...
        'valor_unitario':      df['valor_calculado'],
        'descuento':           df['descuento_valor'],
        'tipo_descuento':      df['tipo_descuento'],
        'es_usd':              df['es_usd'],
    })
    reporte_usd = reporte[reporte['es_usd']].drop(columns=['es_usd']).reset_index(drop=True)
    reporte_bs  = reporte[~reporte['es_usd']].drop(columns=['es_usd']).reset_index(drop=True)
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        if not reporte_usd.empty:
            _escribir_hoja_reporte_con_tipo(
                workbook, "Ventas USD", reporte_usd,
                header_format, percent_format, '$#,##0.00', bold_format
            )
        if not reporte_bs.empty:
...
ORDEN_TIPOS = ['sellout', 'ct_lineal', 'farmago', 'farmatencion', 'dia_laboratorio']
        ...
        if st.button("🚀 Generar Consolidado", type="primary"):
            try:
                ...
                with st.spinner("Consultando Odoo..."):
                    ...
                    # ── Unificar nombre visible de laboratorios con alias ──
                    # Si el producto tiene laboratory_name = "Farma, S.A." o
                    # "Laboratorios Farma, S.A." (o cualquier variante definida
                    # en ALIAS_LABORATORIOS), se muestra siempre con el nombre
                    # canónico en el reporte/Sheets/correos, aunque en Odoo el
                    # producto tenga la variante corta o larga.
                    def _unificar_lab_visible(x):
                        if not isinstance(x, (list, tuple)):
                            return x
                        clave = _lab_key_con_alias(x[1])
                        nombre_canonico = NOMBRES_CANONICOS.get(clave)
                        if nombre_canonico:
                            return (x[0], nombre_canonico)
                        return x

                    df_final['laboratory_name'] = df_final['laboratory_name'].apply(_unificar_lab_visible) # trae product_tmpl_id_int
                    ...
                    # Filas SellOut: una por cada id que matcheó SellOut
                    filas_so = pd.DataFrame()
                    if not df_so_match.empty:
                        filas_so = df_final.merge(df_so_match[['id', 'descuento_so']], on='id', how='inner')
                        filas_so['descuento_valor'] = filas_so['descuento_so']
                        filas_so['fuente_final']    = 'so'
                        filas_so['tipo_reporte']     = 'sellout'

                    # Filas CT: una por cada esquema que matcheó (hasta 2 por id: CT Fijo + Día Laboratorio)
                    filas_ct = pd.DataFrame()
                    if not df_ct_match.empty:
                        filas_ct = df_final.merge(df_ct_match[['id', 'descuento_ct', 'tipo_ct']], on='id', how='inner')
                        filas_ct['descuento_valor'] = filas_ct['descuento_ct']
                        filas_ct['fuente_final']     = 'ct'
                        filas_ct['tipo_reporte']      = filas_ct['tipo_ct']

                    # Descuento SellOut de referencia por factura-línea, para invertir el
                    # precio base de las filas CT (viaja el precio, igual que antes)
                    so_ref = (
                        df_so_match[['id', 'descuento_so']].drop_duplicates(subset=['id'])
                        .rename(columns={'descuento_so': 'descuento_so_ref'})
                        if not df_so_match.empty else pd.DataFrame(columns=['id', 'descuento_so_ref'])
                    )
                    if not filas_ct.empty:
                        filas_ct = filas_ct.merge(so_ref, on='id', how='left')

                    df_bruto = pd.concat([p for p in [filas_so, filas_ct] if not p.empty], ignore_index=True)
    ...
    if st.session_state.df_resultado is not None:
        ...
        for lab in labs_en_resultado:
            ...
            with st.expander(f"📦 {lab}"):
                df_lab_todo = df_display[df_display['laboratory_name'] == lab]

                # ── Botón "todo junto" ────────────────────────────────
                excel_todo = generar_excel_lab_combinado(df_lab_todo, st.session_state.config_lab)
                if excel_todo:
                    safe_lab = _slug_lab(lab)
                    st.download_button(
                        label=f"📦 Todo {lab} ({len(df_lab_todo)} líneas) — con columna Tipo de Descuento",
                        data=excel_todo,
                        file_name=(
                            f"{safe_lab}_Descuentos_Negociados_del_"
                            f"{fecha_inicio.strftime('%d-%m-%Y')}_al_"
                            f"{fecha_fin.strftime('%d-%m-%Y')}.xlsx"
                        ),
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key=f"dl_todo_{lab_key}",
                        use_container_width=True,
                    )
                st.divider()
                cols = st.columns(5)
                for idx, tipo in enumerate(['sellout', 'ct_lineal', 'farmago', 'farmatencion', 'dia_laboratorio']):
...
def _recalcular_resultado(limpiar, limpiar_barcode):
    ...
    st.session_state.excel_binario = generar_excel_unico(res, st.session_state.config_lab)#modulo_consolidado
# ─────────────────────────────────────────────────────────────────
# Combina la lógica de modulo_general_libre y modulo_general_excluyente
# en un único flujo, con configuración de comportamiento por laboratorio:
#
